...
- Add integration tests with a mocked Hacker News server.
- Add load test artifacts (`k6`) and p95 latency reports.
- Add distributed lock for warmup in multi-instance deployments.
- Add structured telemetry (OpenTelemetry + traces/metrics).
- Add explicit stale-while-revalidate behavior and cache versioning.
